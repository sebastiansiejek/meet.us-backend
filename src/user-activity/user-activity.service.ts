...
export class UserActivityService {
    ...
    saveParticipantActivity(type, user, event) {
        ...
        // this.createOrUpdate(user, action, event.type, null, score, weight);

        this.saveTagsActivity(user, action, event.tags, score, weight);

        // this.createOrUpdate(user, actionType.Category, event.type, null, 1, 0.03);

        // this.createOrUpdate(user, actionType.Duration, event.type, null, this.checkDateScore(event), 0.02);
    }

    async saveTagsActivity(user: any, action: actionType, tags: any, score: number, weight: number) {
        if (tags != null) {
            if (tags.length > 0) {
    ...
    async createOrUpdate(
        ...
        console.log('createOrUpdate tag before UserActivitySave', tag);
        ...
        if (!userActivity) {
            console.log('await save userActivity');
                
            console.log(await this.userActivityRepository
                .createQueryBuilder()
                .insert()
                .into(UserActivity)
                .values(activity)
            ...
            console.log( ` user = "${user.id}"  AND actionType = ${userActivity.actionType} AND eventType = ${userActivity.eventType} AND tag = "${userActivity.tag}" AND score = ${userActivity.score} AND weight = ${userActivity.weight}`);
            await this.userActivityRepository
                ...
                .where(
                    ` user = "${user.id}"  AND actionType = ${userActivity.actionType} AND eventType = ${userActivity.eventType} AND tag = "${userActivity.tag}" AND score = ${userActivity.score} AND weight = ${userActivity.weight}`,
                )
                .execute();
        }
    }
    ...
    async saveDistanceSerchedQuery(userId: string, distance: number) {
        ...
        this.createOrUpdate(user, actionType.Distance, null, null, score, 0.03);
    }
    ...
    async saveRateActivity(rate: number, user: User, event: any) {
        ...
        this.createOrUpdate(user, actionType.Rate, event.type, null, rate, 0.04);
        this.saveTagsActivity(user, actionType.Rate, event.tags, rate, 0.02);
    }

    saveEventView(user: User, event: Event) {
        this.createOrUpdate(user, actionType.Visit, event.type, null, 1, 0.01);
