...
export class UserActivityService {
    ...
    async generateQuery(user: User, distanceQuery: string) {
        ...
        const maxGoingCount = await this.getMaxGoingCount();
        const maxInterestedCount = await this.getMaxInterestedCount();
        ...
        for (const activity of activities) {
            ...
            if (activity.actionType === actionType.Category) {
                console.log(0, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (categoryMaxCount < activity.count) categoryMaxCount = activity.count;
            }
            if (activity.actionType === actionType.Duration) {
                console.log(1, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (durationMaxCount < activity.count) durationMaxCount = activity.count;
            }
            if (activity.actionType === actionType.TakePart) {
                console.log(2, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (takePartMaxCount < activity.count) takePartMaxCount = activity.count;
            }
            if (activity.actionType === actionType.Interested) {
                console.log(3, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (interestedMaxCount < activity.count) interestedMaxCount = activity.count;
            }
            if (activity.actionType === actionType.Distance) {
                console.log(4, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (distanceMaxCount < activity.count) distanceMaxCount = activity.count;
            }
            if (activity.actionType === actionType.Rate) {
                console.log(5, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
                if (rateMaxCount < activity.count) rateMaxCount = activity.count;
            }
            if (activity.actionType === actionType.Visit) {
                console.log(6, categoryMaxCount, categoryMaxCount < activity.count, activity.count);
        ...
        for (const activity of activities) {
            if (activity.actionType === actionType.Category) {
                ...
                if (categoryMaxCount > 0) {
                    query += `+ IF(events.type = ${activity.eventType}, IF(${categoryMaxCount}/${activity.count} = 1, 10, IF(${categoryMaxCount}/${activity.count} >= 0.5, 5, IF(${categoryMaxCount}/${activity.count} < 0.5, 3, 0))), 0)`;
                }
            }
            if (activity.actionType === actionType.Duration) {
                ...
                query += `+ IF( time_to_sec(timediff(endDate , startDate )) / 3600 < 3, 10 , IF(time_to_sec(timediff(endDate , startDate )) / 3600 > 7, 3, 5))`;
            }
            if (activity.actionType === actionType.TakePart) {
                query += ` IF(events.type = ${activity.eventType}, ${activity.count} * ${activity.score} * ${activity.weight}, 0) `;
                if (takePartMaxCount > 0) {
                    query += `+ IF(events.type = ${activity.eventType}, IF(${takePartMaxCount}/${activity.count} = 1, 10, IF(${takePartMaxCount}/${activity.count} >= 0.5, 5, IF(${takePartMaxCount}/${activity.count} < 0.5, 3, 0))), 0)`;
                }
            }
            if (activity.actionType === actionType.Interested) {
                query += ` IF(events.type = ${activity.eventType}, ${activity.count} * ${activity.score} * ${activity.weight}, 0) `;
                if (interestedMaxCount > 0) {
                    query += `+ IF(events.type = ${activity.eventType}, IF(${interestedMaxCount}/${activity.count} = 1, 10, IF(${interestedMaxCount}/${activity.count} >= 0.5, 5, IF(${interestedMaxCount}/${activity.count} < 0.5, 3, 0))), 0)`;
                }
            }
            if (activity.actionType === actionType.Distance) {
            ...
            if (activity.actionType === actionType.Rate) {
                query += ` IF(events.type = ${activity.eventType}, ${activity.count} * ${activity.score} * ${activity.weight}, 0) `;
                if (rateMaxCount > 0) {
                    query += `+ IF(events.type = ${activity.eventType}, IF(${rateMaxCount}/${activity.count} = 1, 10, IF(${rateMaxCount}/${activity.count} >= 0.5, 5, IF(${rateMaxCount}/${activity.count} < 0.5, 3, 0))), 0)`;
                }
            }
            if (activity.actionType === actionType.Visit) {
                query += ` IF(events.type = ${activity.eventType}, ${activity.count} * ${activity.score} * ${activity.weight}, 0) `;
                if (visitMaxCount > 0) {
                    query += `+ IF(events.type = ${activity.eventType}, IF(${visitMaxCount}/${activity.count} = 1, 10, IF(${visitMaxCount}/${activity.count} >= 0.5, 5, IF(${visitMaxCount}/${activity.count} < 0.5, 3, 0))), 0)`;
                }
            }
    ...
    async getMaxInterestedCount() {
        const participants = await this.entityManager.query(`SELECT count(event) as interestedCount FROM participants WHERE type = 1 GROUP BY event ORDER BY interestedCount DESC LIMIT 1`);
        
        for(const participant of participants){
            return participant.interestedCount
        }
        return 0;
    }
    async getMaxGoingCount() {
        const participants = await this.entityManager.query(`SELECT count(event) as goingCount FROM participants WHERE type = 2 GROUP BY event ORDER BY goingCount DESC LIMIT 1`);
        
        for(const participant of participants){
            return participant.goingCount
        }
        return 0;
    }
    async getMaxVisitCount() {
        const events = await this.entityManager.query(`select visitCount FROM events order by visitCount DESC  LIMIT 1`);
