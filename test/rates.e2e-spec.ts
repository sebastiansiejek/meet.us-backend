...
describe('Rate (e2e)', () => {
  ...
  it('Participatte in event', async () => {
    ...
    const participateResponse = await request('http://localhost:3000')
      .post('/graphql')
    ...
    expect(participateResponse.body.errors).toBeUndefined();
    expect(participateResponse.body.data?.participateInEvent.user.id).toBe(
      newUser.body.data?.createUser.id,
    );
    expect(participateResponse.body.data?.participateInEvent.event.id).toBe(
      newEvent.id,
    );
    ...
    expect(response.body.data?.rateEvent.user.id).toBe(
      newUser.body.data?.createUser.id,
    );
    expect(response.body.data?.rateEvent.event.id).toBe(newEvent.id);
  });
